...
{
    int resv = 0;
    int buff_size = nmea_property()->parse_buff_size;		//max 1024 , min 256
...
int nmea_parser_real_push(nmeaPARSER *parser, const char *buff, int buff_sz)
{
    ...
    /* add */
    if(parser->buff_use + buff_sz >= parser->buff_size)		//若添加新的buff数据会超出上限，则清空buff
        nmea_parser_buff_clear(parser);

    memcpy(parser->buffer + parser->buff_use, buff, buff_sz);	//添加新数据
    parser->buff_use += buff_sz;

    /* parse */
    for(;;node = 0)
    {
        sen_sz = nmea_find_tail(								//查找语句尾	
...
int nmea_parser_push(nmeaPARSER *parser, const char *buff, int buff_sz)
{
    ...
    do
    {
        if(buff_sz > parser->buff_size)		//若传入的数组大于nmea解码用的数组（1024），直接用1024大小一点点传入数据进行处理
            nparse = parser->buff_size;
        else
            nparse = buff_sz;				//若传入的数组比nmea解码数组小
